...
> Note: Accuracy reported from Colab training validation (epoch 3). AUC computed on CPU baseline model. Local evaluation shows lower numbers due to a `sentence-transformers` version mismatch between Colab (newer) and local environment.
